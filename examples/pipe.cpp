...
int main()
{
	...
	auto f1 = pipe(mulLambda, addThreeFunctor);

	auto f2 = pipe(mul, addThree, multByFour, toString);

	auto f3 = pipe(mulFunctor, addThree);
	...
	auto f5 = pipe(mul, std::bind(mul, std::placeholders::_1, 2));

	std::cout << f1(5, 7) << std::endl // 38
